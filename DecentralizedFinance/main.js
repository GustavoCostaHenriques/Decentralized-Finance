...
const defi_contractAddress = "0xccEe5c40a7C8088b5d08490785e4a560AE24adfD";
...
const nft_contractAddress = "0xfaa72607FFba7BF45A9678cd6d3E11f619a48B7e";
...
async function checkLoanStatus() {
    ...
    for (let loanId = 1; loanId <= maxLoanId; loanId++) {
        try {
            const paymentFailed = await defi_contract.methods.checkLoan(loanId).call({ from: account });

            if (paymentFailed) {
                console.log(`Loan ${loanId} has missed payments or deadline passed.`);
